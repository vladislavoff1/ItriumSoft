...
public class SetStatus implements Method {

    private static String queryGetID = "SELECT * FROM  `Users` WHERE  `email` LIKE  '%email' AND  `password` LIKE  '%password'";
    private static String query = "INSERT INTO `Statuses`(`id`, `status`) VALUES (\"%id\", \"%status\") ON DUPLICATE KEY UPDATE `status` = \"%status\";";
    ...
    public String run(Map<String, String[]> args) {
        ...
        try {
            ...
            Statement statement = Database.createStatement();
            String q = query.replaceAll("%id", key).replaceAll("%status", status);
