...
public class GetEvents implements Method {
    ...
    private static String query = "SELECT *  FROM `Events` WHERE %find ORDER BY `Events`.`time` ASC LIMIT 0, 30";
    ...
    public String run(Map<String, String[]> args) {
        ...
        try { 
            ...
            for (int i = 0; i < size; i++) {
                ...
                item.put("msg", result.getString("msg"));
                item.put("time", result.getString("time"));
                item.put("priority", result.getInt("priority") + "");
