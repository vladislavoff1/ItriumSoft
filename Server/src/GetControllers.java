...
public class GetControllers implements Method {

    private static String queryGetID = "SELECT * FROM  `Users` WHERE  `email` LIKE  '%email' AND  `password` LIKE  '%password'";
    private static String query = "SELECT * FROM `Statuses` WHERE `id` in (SELECT `privateKey` FROM `Controllers` WHERE `user` = %user)";
	...
	public String run(Map<String, String[]> args) {
        ...
        try { 
            ...
            Statement statement = Database.createStatement();
            String q = query.replaceAll("%user", id + "");
            ResultSet result = statement.executeQuery(q);
            ...
            for (int i = 0; i < size; i++) {
                ...
                item.put("key", result.getString("id"));
                item.put("time", result.getString("time"));
